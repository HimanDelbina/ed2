...
class SalonManagerReportUserPage extends StatefulWidget {
  const SalonManagerReportUserPage({super.key});

  @override
  State<SalonManagerReportUserPage> createState() =>
      _SalonManagerReportUserPageState();
}

class _SalonManagerReportUserPageState
    extends State<SalonManagerReportUserPage> {
  int? id_user = 0;
  int? unit_id = 0;
  bool? is_manager;
      ...
      child: Column(
        ...
        children: [
          ...
          leave.isDays!
              ...
              : leave.finalAccept!
                  ? const Text("مرخصی شما تاییدیه نهایی شد .",
                      style: TextStyle(
                          fontWeight: FontWeight.bold, color: Colors.green))
                  : const SizedBox(),
        ],
  ...
  Future get_all_data_filter() async {
    String infourl = Helper.url.toString() +
        'all_data/combined_data_salonManager_report/?date_filter=' +
        url_select.toString() +
        '&status_filter=' +
