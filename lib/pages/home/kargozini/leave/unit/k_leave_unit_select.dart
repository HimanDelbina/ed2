...
import '../../../../../models/leave/leave_report_unit_model.dart';
...
class KargoziniLeaveUnitSelect extends StatefulWidget {
  int? unit_id;
  KargoziniLeaveUnitSelect({super.key, this.unit_id});

  @override
  State<KargoziniLeaveUnitSelect> createState() =>
      _KargoziniLeaveUnitSelectState();
}

class _KargoziniLeaveUnitSelectState extends State<KargoziniLeaveUnitSelect> {
  @override
  void initState() {
    super.initState();
    get_unit_data();
  ...
  Future get_unit_data() async {
    is_all_filter!
        ? infourl = Helper.url.toString() +
            'leave/calculate_leaves_by_unit/' +
    ...
    //  infourl = Helper.url.toString() +
    //     'leave/calculate_leaves_by_unit/${widget.unit_id}';
    ...
    if (response.statusCode == 200) {
      var x = response.body;
      var recive_data = leaveReportUnitModelFromJson(x);
      setState(() {
  ...
  Future<void> exportToExcel(List<dynamic> data) async {
    ...
    if (fileBytes != null) {
      // ذخیره فایل اکسل
      await saveFile('unit_report.xlsx', fileBytes);
    }
  }
