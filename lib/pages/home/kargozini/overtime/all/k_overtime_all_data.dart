...
import 'package:ed/models/overtime/overtime_get_all_model.dart';
import 'package:ed/static/helper_page.dart';
import 'package:http/http.dart' as http;
import '../../../../../static/excel_save.dart';
import 'package:pdf/pdf.dart';
import 'package:pdf/widgets.dart' as pw;
import 'package:printing/printing.dart';
import 'package:flutter/services.dart';

class KargoziniOvertimeGetAllUserPage extends StatefulWidget {
  const KargoziniOvertimeGetAllUserPage({super.key});

  @override
  State<KargoziniOvertimeGetAllUserPage> createState() =>
      _KargoziniOvertimeGetAllUserPageState();
}

class _KargoziniOvertimeGetAllUserPageState
    extends State<KargoziniOvertimeGetAllUserPage> {
  @override
  void initState() {
    super.initState();
    get_overtime_data();
  ...
  Future get_overtime_data() async {
    is_all_filter!
        ? infourl = Helper.url.toString() +
            'overtime/get_users_overtime_summary' +
            '?select=' +
            select_ez!
        : is_start_date!
            ? infourl = Helper.url.toString() +
    ...
    // String infourl =
    // Helper.url.toString() + 'overtime/get_users_overtime_summary';
    ...
    if (response.statusCode == 200) {
      var x = response.body;
      var recive_data = ovretimeReportAllModelFromJson(x);
      setState(() {
  ...
  Future<void> exportToExcel(List<dynamic> data) async {
    ...
    if (fileBytes != null) {
      // ذخیره فایل اکسل
      await saveFile('all_overtime.xlsx', fileBytes);
    }
  }
  ...
  Future<void> generatePdf(List<User> data) async {
    ...
    final fontData = await rootBundle.load("fonts/BZar.ttf");
    print('Font loaded: ${fontData.lengthInBytes}');
              ...
              pw.Table.fromTextArray(
                ...
                data: <List<String>>[
                  ...
                  ...data.map((user) {
                    ...
                    return [
                      user.name ?? '',
                      overtime.isNotEmpty ? overtime : '0',
                      holiday.isNotEmpty ? holiday : '0',
                      friday.isNotEmpty ? friday : '0',
                      mission.isNotEmpty ? mission : '0',
                    ];
                  }).toList(),
                ],
              ),
    ...
    await savePdfFile('overtime_report.pdf', pdfBytes);
  }
