...
class ReportShiftUnitSelect extends StatefulWidget {
  final int? unitId;

  ReportShiftUnitSelect({super.key, this.unitId});

  @override
  _ReportShiftUnitSelectState createState() => _ReportShiftUnitSelectState();
}

class _ReportShiftUnitSelectState extends State<ReportShiftUnitSelect> {
      ...
      children: [
        ...
        if (user.shiftReport != null)
          ...user.shiftReport!.keys.map<TableRow>((month) {
            ...
            return TableRow(
              children: [
                _buildTableCell(month ?? 'نامشخص'),
                _buildTableCell(report?.totalShifts?.toString() ?? '0'),
                _buildTableCell(report?.confirmedShifts?.toString() ?? '0'),
                _buildTableCell(shiftTypes?.so?.toString() ?? '0'),
                _buildTableCell(shiftTypes?.shiftTypesAs?.toString() ?? '0'),
                _buildTableCell(shiftTypes?.sh?.toString() ?? '0'),
              ],
            );
          }).toList(),
      ],
  ...
  Future<void> getShiftAllUnit() async {
    ...
    String url = '${Helper.url}/shift/get_all_shift_unit/${widget.unitId}';
    try {
