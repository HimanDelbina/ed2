...
import '../../../../services/car_plate_display_table.dart';
import '../../../../static/helper_page.dart';

class ManagerAnbarImportReportTable extends StatefulWidget {
  const ManagerAnbarImportReportTable({super.key});

  @override
  State<ManagerAnbarImportReportTable> createState() =>
      _ManagerAnbarImportReportTableState();
}

class _ManagerAnbarImportReportTableState
    extends State<ManagerAnbarImportReportTable> {
  @override
  void initState() {
    super.initState();
    get_all_commodity();
                ...
                columns: const [
                  ...
                  DataColumn(
                      label: Text('نوع',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('تاریخ درخواست',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('نام کالا',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('تعداد / وزن',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('تاریخ تایید انبار',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('نوع خودرو',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('شماره تماس راننده',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                  DataColumn(
                      label: Text('پلاک خودرو',
                          style: TextStyle(fontWeight: FontWeight.bold))),
                ],
                rows: data!.map((item) {
                  return DataRow(cells: [
                    DataCell(Text(item.company.name.toString())),
                    DataCell(Text(item.select == "M" ? "مواد اولیه" : "کالا")),
                    DataCell(Text(
                        "${FormateDateCreate.formatDate(item.createAt.toString())}")),
                    DataCell(Text(item.name)),
                    DataCell(Text(item.weightCommodity != null
                        ? " ${item.weightCommodity.toString().toPersianDigit()}"
                        : " ${item.countCommodity.toString().toPersianDigit()}")),
                    DataCell(Text(item.anbarDate == null
                        ? "انبار هنوز تایید نکرده"
                        : "${FormateDateCreateChange.formatDate(item.anbarDate.toString())}")),
                    DataCell(Text(item.car.name)),
                    DataCell(Text(item.carPhone.toString().toPersianDigit())),
                    DataCell(CarPlateDisplayTable(carPlate: item.carPlate)),
                  ]);
                }).toList(),
              ),
            ),
