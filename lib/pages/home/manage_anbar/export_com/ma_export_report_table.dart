...
import 'package:ed/services/car_plate_display_table.dart';
import '../../../../services/car_plate_display.dart';
import '../../../../static/helper_page.dart';

class ManagerAnbarExportReportTable extends StatefulWidget {
  const ManagerAnbarExportReportTable({super.key});

  @override
  State<ManagerAnbarExportReportTable> createState() =>
      _ManagerAnbarExportReportDataState();
}

class _ManagerAnbarExportReportDataState
    extends State<ManagerAnbarExportReportTable> {
  @override
  void initState() {
    super.initState();
                    ...
                    : SingleChildScrollView(
                        ...
                        child: DataTable(
                          ...
                          columns: const [
                            ...
                            DataColumn(label: Text('نوع درخواست')),
                            DataColumn(label: Text('تاریخ درخواست')),
                            DataColumn(label: Text('نام کالا')),
                            DataColumn(label: Text('تعداد / وزن')),
                            DataColumn(label: Text('تاییدیه نگهبانی')),
                            DataColumn(label: Text('دریافت کننده/خریدار')),
                            DataColumn(label: Text('پلاک خودرو')),
                          ],
                          rows: data!.map((item) {
                            // استخراج مقدار تعداد یا وزن
                            final itemQuantity = item.count != null
                                ? "${item.count.toString().toPersianDigit()} عدد"
                                : "${item.weight.toString().toPersianDigit()} کیلو";
                            ...
                            return DataRow(
                              cells: [
                                ...
                                DataCell(
                                  Text(
                                    FormateDateCreateChange.formatDate(
                                ...
                                DataCell(
                                  Text(item.name),
                                ), // ستون برای نمایش نام کالا
                                DataCell(
                                  Text(itemQuantity),
                                ), // ستون برای نمایش تعداد یا وزن
                                DataCell(
                                  Text(
                                    item.isGuard
                                        ? FormateDateCreateChange.formatDate(
                                            item.guardDate.toString())
                                        : "هنوز نگهبانی تایید نکرده",
                                    style: TextStyle(
                                        color: item.isGuard
                                            ? Colors.green
                                            : Colors.red,
                                ...
                                DataCell(
                                  CarPlateDisplayTable(carPlate: item.carPlate),
                                ),
                              ],
                            );
                          }).toList(),
