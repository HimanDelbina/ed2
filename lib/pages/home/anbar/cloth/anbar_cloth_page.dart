...
import 'package:ed/models/anbar/cloth_model.dart';
import 'package:ed/static/helper_page.dart';
import 'package:flutter/material.dart';
import 'package:shared_preferences/shared_preferences.dart';
import 'package:http/http.dart' as http;

class AnbarClothPage extends StatefulWidget {
  const AnbarClothPage({super.key});

  @override
  State<AnbarClothPage> createState() => _AnbarClothPageState();
}

class _AnbarClothPageState extends State<AnbarClothPage> {
  int? id_user = 0;
  int? id_unit = 0;
  ...
  String? date_only_cloth;
  String? date_only_shoes;
  @override
          ...
          : data!.isEmpty
              ...
              : Column(
                  ...
                  children: [
                    ...
                    Column(
                      ...
                      children: [
                        data_last_cloth == null
                            ? const SizedBox()
                            : Text(
                                "شما در تاریخ ${FormateDateCreateChange.formatDate(data_last_cloth!.clothDate.toString())} آخرین لباس کار خود را دریافت کرده "),
                        data_last_cloth == null
                            ? const SizedBox()
                            : data_last_cloth.daysLeftCloth <= 0
                                ? const Text(
                                    "شما میتواند لباس کار دریافت کند",
                                    style: TextStyle(
                                        color: Colors.blue,
                        ...
                        data_last_shoes == null
                            ? const SizedBox()
                            : Text(
                                "شما در تاریخ ${FormateDateCreateChange.formatDate(data_last_shoes!.shoesDate.toString())} آخرین کفش کار خود را دریافت کرده "),
                        data_last_shoes == null
                            ? const SizedBox()
                            : data_last_shoes.daysLeftShoes <= 0
                                ? const Text(
                                    "شما میتواند کفش کار دریافت کند",
                                    style: TextStyle(
                                        color: Colors.blue,
                                        fontWeight: FontWeight.bold),
                                  )
                                : Text(
                                    "کفش کار : ${data_last_shoes.daysLeftShoes.toString().toPersianDigit()} روز مانده ")
                      ],
                    ),
                    // const Padding(
                    ...
                    //       onlyclothDate = clotheDateString.split(' ')[0];
                    //       onlyshoesDate = shoesDateString.split(' ')[0];
                    //       return Container(
  ...
  Future get_clothe_by_user_id() async {
    String infourl =
        Helper.url.toString() + 'anbar/get_cloth_by_user_id/${id_user}';
