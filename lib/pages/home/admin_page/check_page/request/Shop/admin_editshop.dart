...
class AdminEditShopPage extends StatefulWidget {
  List<dynamic>? data;
  int? anbar_id;
  AdminEditShopPage({super.key, this.data, this.anbar_id});

  @override
  State<AdminEditShopPage> createState() => _ManagerShopState();
}

class _ManagerShopState extends State<AdminEditShopPage> {
  ...
  Future edit_shop() async {
    List<Map<String, dynamic>> shopDataList =
        widget.data!.map((item) => (item as ShopDatum).toJson()).toList();

    var body = jsonEncode({"shop_data": shopDataList});

    String infourl =
        Helper.url.toString() + 'anbar/edit_shop_admin/${widget.anbar_id}/';
